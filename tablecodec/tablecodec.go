...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	if !checkSlicesSame(key[0:tablePrefixLength], tablePrefix) {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid record key: %q", key)
	}
	...
	// consider key error
	if !checkSlicesSame(key[0:recordPrefixSepLength], recordPrefixSep) {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid record key: %q", key)
	}
...
// check equality of two slice
func checkSlicesSame(slice1 []byte, slice2 []byte) bool {
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	...
	// consider key error
	if len(key) < tablePrefixLength {
		return 0, 0, nil, errInvalidRecordKey.GenWithStack("invalid record key: %q", key)
	}
	if !checkSlicesSame(key[0:tablePrefixLength], tablePrefix) {
		return 0, 0, nil, errInvalidRecordKey.GenWithStack("invalid record key: %q", key)
	}
	...
	// consider key error
	if !checkSlicesSame(key[0:len(indexPrefixSep)], indexPrefixSep) {
		return 0, 0, nil, errInvalidRecordKey.GenWithStack("invalid record key: %q", key)
	}
