...
func (b *PlanBuilder) pushTableHints(hints []*ast.TableOptimizerHint) {
	...
	for _, hint := range hints {
		...
		case TiDBIndexNestedLoopJoin, HintINLJ:
			INLJTables = append(INLJTables, tableNames2HintTableInfo(b.ctx, hint.Tables)...)
		case HintINLHJ:
			INLHJTables = append(INLHJTables, tableNames2HintTableInfo(b.ctx, hint.Tables)...)
		case HintINLMJ:
...
func (b *PlanBuilder) buildUpdate(ctx context.Context, update *ast.UpdateStmt) (Plan, error) {
	...
	if update.Order != nil {
		p, err = b.buildSort(ctx, p, update.Order.Items, nil)
		if err != nil {
			return nil, err
		}
	}
...
func (b *PlanBuilder) buildUpdateLists(
	...
	for i, assign := range allAssignments {
		...
		if i < len(list) {
			...
			newExpr, np, err = b.rewriteWithPreprocess(ctx, assign.Expr, p, nil, false, rewritePreprocess)
		}
		if err != nil {
			return nil, nil, false, err
		}
